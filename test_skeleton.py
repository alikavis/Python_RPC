...
import numbers, json, jsonpickle, Pyro4, socket, select, exceptions, sys, threadin
...
class test_skeleton:
    def __init__(self, server_instance, server_name):
        self.server_instance = server_instance
        self.request_set = set()
        self.timeout = timeout
        self.retry_count = retry_count

        self.serversocket = self.init_stub(server_name)

    def init_stub(self, servername):
    ...
    def receive_request(self, client, address):
        ...
        if str(len(json_obj['param'])) == payload_size
            ...
            if params[0] in self.request_set:
                raise Exception(0, 'Request has already been received, rolling back...')
            ...
            try:
                self.send_response(client, func, marshalled_param)
    ...
    def send_response(self, client, func_name, serialized_data):
        ...
        while sent_amount < msg_len:
            ...
            if temp == 0:
                raise IOError('Client socket disconnected')
            sent_amount += temp
    ...
    def run(self):
        ...
        while 1:
            ...
            threading.Thread(target=receive_request, args=(client, address)).start()
                ...
                print 'Server detected recurrent call:\n{0}: {1}'.format(err[0], err[1])
